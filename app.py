...
def SaveInfoToDBProduct(WorkerId, Sort, GloveCount, ComingProductId, ProductId):
    ...
    Id=DBCursor.fetchone()[0]
    Id=Id+1 if Id != None else 0
    DBCursor.execute(f"""INSERT INTO workers_gloves_quantity VALUES ({Id}, {WorkerId}, '1', '{UsersFlows[WorkerId]['Stage'].replace("'", "''")}', '{ProductFullName}', {Sort}, {int(GloveCount)/2}, '{str(datetime.now(pytz.timezone('Europe/Kiev')).strftime("%d.%m.%Y %H:%M"))}')""")

    DBCursor.execute(f"SELECT MAX(Id) FROM products_gloves_quantity")
    Id=DBCursor.fetchone()[0]
    Id=Id+1 if Id != None else 0
    DBCursor.execute(f"""INSERT INTO products_gloves_quantity VALUES ({Id}, {ProductId}, '{UsersFlows[WorkerId]['Stage'].replace("'", "''")}', '1', {Sort}, {int(GloveCount)/2}, '{str(datetime.now(pytz.timezone('Europe/Kiev')).strftime("%d.%m.%Y %H:%M"))}')""")
    ...
    ComePair=DBCursor.fetchone()[0]-(int(GloveCount)/2)
    DBCursor.execute(f"""UPDATE comings SET Pair={ComePair} WHERE Id={ComeId}""")

    DBCursor.execute(f"SELECT MAX(Id) FROM comings_info")
    Id=DBCursor.fetchone()[0]
    Id=Id+1 if Id != None else 0
    DBCursor.execute(f"""INSERT INTO comings_info VALUES ({Id}, {ComeId}, '{UsersFlows[WorkerId]['Stage'].replace("'", "''")}', '{UsersFlows[WorkerId]['Worker']}', '{ProductFullName}', {int(GloveCount)/2}, {Sort}, '{str(datetime.now(pytz.timezone('Europe/Kiev')).strftime("%d.%m.%Y %H:%M"))}')""")
...
def ShowProductsMachine(WorkerId):
    ...
    ShiftName=f"Відлік зміни {UsersFlows[WorkerId]['Worker']}, {UsersFlows[WorkerId]['Stage']}"
...
def ComingProductSelect(WorkerId):
    ...
    if request.method == 'POST' and request.form['StopShift']=='0':
        ...
        Products=sorted(map(lambda Element: f"{Element[0]} ({int(Element[1]*2)} шт.)", DBCursor.fetchall()))
...
def AddGlovesInProduct(WorkerId, ComingProductId, ProductId):
    ...
    if request.method == 'POST':
        ...
        ComingProductCounts = DBCursor.fetchone()[0]*2
        CloseDB()
...
def ShowProductsProduct(WorkerId):
    ...
    for ProductsId, ProductCount in UsersFlows[WorkerId]['GlovesCount'].items():
        ...
        for Sort, Count in ProductCount.items():
            if Count != 0:
                CountdownInfo.append((ComingProductName, ProductName, Sort.replace("FirstSort", "1").replace("SecondSort", "2").replace("DefectSort", "3"), Count/2))
...
def ComingProductCountSet(WorkerId):
    ...
    if request.method == 'POST':
        ...
        if (UsersFlows[WorkerId]["AddedComingProduct"], UsersFlows[WorkerId]['Stage'].replace("'", "''")) not in Comings:
            ...
            Id=DBCursor.fetchone()[0]
            Id=Id+1 if Id != None else 0
            DBCursor.execute(f"""INSERT INTO comings VALUES ({Id}, '{UsersFlows[WorkerId]['Stage'].replace("'", "''")}', '{UsersFlows[WorkerId]["AddedComingProduct"]}', {ComingProductCount/2}, '{str(datetime.now(pytz.timezone('Europe/Kiev')).strftime("%d.%m.%Y %H:%M"))}', '?')""")
        
            if ComingProductCount/2==0:
            ...
            ComeId=DBCursor.fetchone()[0]
            DBCursor.execute(f"""SELECT Pair FROM comings WHERE Id={ComeId}""")
            ComePair=DBCursor.fetchone()[0]+(ComingProductCount/2)
            DBCursor.execute(f"""UPDATE comings SET Pair={ComePair} WHERE Id={ComeId}""")
